export function VibrantWallpaper() {
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none z-0 bg-[#f0f0f5]">
      {/* will-change keeps each blob on its own compositing layer, so the heavy
          blur is rasterized once and only transformed as it drifts. Without it,
          every frame re-rasterizes a viewport-sized blurred layer, which the
          glass surfaces then re-sample — that thrash shows up as flicker. */}
      <div
        className="animate-ambient-drift absolute top-[-20%] left-[-10%] w-[70%] h-[70%] rounded-[100%] opacity-80 mix-blend-multiply blur-[100px]"
        style={{
          background: "radial-gradient(ellipse, #ffb199 0%, #ff0844 100%)",
      ...
      <div
        ...
        style={{
          background: "radial-gradient(ellipse, #4facfe 0%, #00f2fe 100%)",
          willChange: "transform",
        }}
      />
      <div
        ...
        style={{
          background: "radial-gradient(ellipse, #fbc2eb 0%, #a18cd1 100%)",
          willChange: "transform",
        }}
      />
      <div
